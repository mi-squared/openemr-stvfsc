...
if ($_POST['bn_save']) {
 ...
 if ($formid) {
  $query = "DELETE FROM form_ros_exam_mi2 WHERE forms_id = '$formid'";
  sqlStatement($query);
 }
 else {
  $formid = addForm($encounter, "Review of Systems", 0, "ros_exam_mi2", $pid, $userauthorized);
 ...
 foreach ($form_obs as $line_id => $line_array) {
  ...
  if ($yes || $no || $comments) {
   $query = "INSERT INTO form_ros_exam_mi2 ( " .
...
$rows = array();
if ($formid) {
 $res = sqlStatement("SELECT * FROM form_ros_exam_mi2 WHERE forms_id = '$formid'");
...
<body class="body_top">
<form method="post" action="<?php echo $rootdir ?>/forms/ros_exam_mi2/new.php?id=<?php echo $formid ?>"
...
<table border='0' width='98%'>
 ...
 <tr>
  <td align='left'   width='1%' nowrap><b><?php xl('System(s) Reviewed','e'); ?></b></td>
 </tr>

<?php
 foreach ($roslines as $sysname => $sysarray) {
