<?php /* Smarty version 2.3.1, created on 2017-02-19 18:44:11
	    ...
	    $fname = substr($event['patient_name'], $commapos + 2);
            ...
            if ($catid == 2 || $catid == 3 || $catid == 4 || $catid == 8 || $catid == 11) {
                ...
                if ($patientid) {
                    ...
                    $status_class="status".ord($event['apptstatus'][0]);  // KHY - convert the character to a number for use as a valid class name.  
                    $content .= "<a href='javascript:goPid($patientid)' title='$link_title' class='$status_class'>";
                    ...
                    if ($catid == 1) $content .= "</strike>";
                    $month_loc=strpos($patient_age,"month"); // KHY - Format the patient age for display on the calendar
                    $age_display =  $month_loc===false ? trim($patient_age)."Y" : substr($patient_age,0,$month_loc-1)."M";
                    $content .= " <span class='patient_age'>".$age_display."</span> ";                
                    $content .= "</a>";
                }
                else {
