...
class Form_Signable extends DbRow_Signable implements SignableIF
{
    ...
    public function getData()
    {
        ...
        //The PK pf some of these tables are not all called id.  This is a work-around.
        //for example, the procedure_order table calls the first column procedure_order_id and this causes sql errors.
        $column = mysql_query('DESCRIBE '.escape_table_name( $table ));
        $col = mysql_fetch_array($column);
        
        // Get form-specific data
        $statement = "SELECT * FROM ".escape_table_name( $table );
        $statement .= " WHERE $col[0] = ? LIMIT 1";
